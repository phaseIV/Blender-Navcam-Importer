...
def ReadNavcamString(inString, inFillBool, inRadBool):
    ...
    SetRenderSettings()
    local_data_dir = os.path.join(bpy.context.user_preferences.filepaths.temporary_directory, 'MarsRoverImages/')
...
def create_mesh_from_depthimage(rover, sol, image_depth_filename, image_texture_filename, do_fill, do_rad):
    ...
    try:
        with open(image_texture_filename):
            ...
            cTex = bpy.data.textures.new('Tex-' + os.path.basename(FileAndExt[0]), type='IMAGE')
            cTex.image = img
